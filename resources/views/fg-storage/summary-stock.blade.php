...
<div class="space-y-4 text-[13px] text-gray-700">
    ...
    <div class="flex flex-wrap gap-1 text-sm">
        ...
        <a href="{{ route('fg.storage.summary-stock') }}" class="rounded border border-yellow-500 bg-yellow-300 px-3 py-2 font-semibold text-gray-800">Summary Stock</a>
    ...
    <div class="flex flex-wrap items-center gap-2">
        <form method="GET" action="{{ route('fg.storage.summary-stock.export') }}" class="flex items-center gap-2">
    ...
    @php
    $currentSortBy = $filters['sort_by'] ?? 'part_code';
    $currentSortDir = strtolower($filters['sort_dir'] ?? 'asc') === 'asc' ? 'asc' : 'desc';
    ...
    return route('fg.storage.summary-stock', array_filter($query, fn($v) => $v !== null && $v !== ''));
    };
    @endphp
    ...
    <div class="overflow-x-auto rounded border border-yellow-300">
        <table class="min-w-full border-collapse text-xs">
            ...
            <tbody class="bg-yellow-50">
                ...
                @empty
                <tr>
                    <td colspan="5" class="border border-yellow-200 px-3 py-4 text-center text-gray-500">
                        No records to view
                    </td>
                </tr>
                @endforelse
