...
<div class="space-y-4 text-[13px] text-gray-700">
    ...
    return route('fg.storage.stock', $query);
    };
    @endphp
    ...
    @endphp
    <form method="GET" action="{{ route('fg.storage.stock') }}" class="rounded-lg border border-gray-200 bg-white px-4 py-3 shadow-sm">
        ...
        <div class="flex flex-wrap items-center gap-3 border-t border-gray-100 pt-2">
            ...
            <input type="text" readonly value="{{ $summary['total_row'] ?? 0 }}"
            ...
            <div class="text-xs text-gray-500">
                Total Qty Stock : <strong class="text-gray-700">{{ number_format($summary['total_qty'] ?? 0) }}</strong>
            </div>
        </div>
    </form>
        ...
        <table class="min-w-full border-collapse text-xs">
            <thead class="bg-yellow-200 text-gray-700">
                <tr>
                    ...
                    <th class="whitespace-nowrap border border-yellow-300 px-2 py-2 text-center">
                        <a href="{{ $sortUrl('qty_box') }}" class="hover:underline">Qty Box</a>
                    </th>
            ...
            <tbody class="bg-yellow-50">
                ...
                <tr>
                    ...
                    <td class="border border-yellow-200 px-2 py-1">{{ $scan->operator?->name ?? '-' }}</td>
                    @if(auth()->user()?->isAdmin())
                    <td class="border border-yellow-200 px-2 py-1 text-center">
                        <div class="flex items-center justify-center gap-1">
                            <button type="button" title="Edit"
                                data-update-url="{{ route('fg.storage.stock.update', $scan) }}"
                            ...
                            <form method="POST" action="{{ route('fg.storage.stock.destroy', $scan) }}"
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="w-full max-w-lg rounded border border-gray-300 bg-white shadow-xl">
            <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3">
                <h3 class="font-semibold text-gray-700">Edit FG Stock</h3>
