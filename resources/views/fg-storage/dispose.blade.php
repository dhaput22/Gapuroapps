...
<div class="space-y-4 text-[13px] text-gray-700">
    ...
    @endphp
    <form method="GET" action="{{ route('fg.storage.dispose.index') }}"
        ...
        <div class="mb-2 flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('fg.storage.dispose.index') }}"
    ...
    <div class="flex flex-wrap gap-1 text-sm">
        ...
        <a href="{{ route('fg.storage.dispose.index') }}" class="rounded border border-yellow-500 bg-yellow-300 px-3 py-2 font-semibold text-gray-800">FG Dispose</a>
    ...
    <div class="flex flex-wrap gap-2">
        <a href="{{ route('fg.storage.dispose.create') }}" class="inline-flex items-center gap-1.5 rounded border border-yellow-500 bg-yellow-300 px-2 py-1 text-sm font-medium text-gray-800 hover:bg-yellow-400">
    ...
    @php
    $currentSortBy = $filters['sort_by'] ?? 'dispose_at';
    ...
    return route('fg.storage.dispose.index', array_filter($query, fn($v) => $v !== null && $v !== ''));
    };
    @endphp
        ...
        <table class="min-w-full border-collapse text-xs">
            <thead class="bg-yellow-200 text-gray-700">
                <tr>
                    ...
                    <th class="whitespace-nowrap border border-yellow-300 px-2 py-2 text-center">
                        <a href="{{ $sortUrl('dispose_at') }}" class="hover:underline">Dispose Date</a>
                    </th>
                    <th class="whitespace-nowrap border border-yellow-300 px-2 py-2 text-center">
                    ...
                    <th class="whitespace-nowrap border border-yellow-300 px-2 py-2 text-center">Dispose By</th>
            ...
            <tbody class="bg-yellow-50">
                ...
                <tr>
                    ...
                    <td class="border border-yellow-200 px-2 py-1">{{ optional($scan->dispose_at)->format('Y-m-d') ?? optional($scan->created_at)->format('Y-m-d') }}</td>
                    ...
                    <td class="border border-yellow-200 px-2 py-1">{{ $scan->disposeOperator?->name ?? '-' }}</td>
                    ...
                    <td class="border border-yellow-200 px-2 py-1 text-center">
                        <div class="flex items-center justify-center gap-1">
                            <button type="button" title="Edit"
                                data-update-url="{{ route('fg.storage.dispose.update', $scan) }}"
                                data-part-code="{{ $scan->part_code }}"
                                data-part-name="{{ $scan->part_name ?? '' }}"
                                data-lot-no="{{ $scan->lot_no }}"
                                data-qty-box="{{ $scan->qty_box }}"
                                data-dispose-at="{{ optional($scan->dispose_at)->format('Y-m-d') }}"
                                data-dispose-operator-id="{{ $scan->dispose_operator_id ?? '' }}"
                                data-remark="{{ $scan->remark ?? '' }}"
                            ...
                            <form method="POST" action="{{ route('fg.storage.dispose.destroy', $scan) }}"
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="w-full max-w-lg rounded border border-gray-300 bg-white shadow-xl">
            <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3">
                <h3 class="font-semibold text-gray-700">Edit FG Dispose</h3>
            ...
            <form id="editModal-form" method="POST"
                ...
                <div class="grid grid-cols-1 gap-3 px-4 py-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Dispose Date</label>
                        <input type="date" name="dispose_at" id="modal-dispose-at" value="{{ old('dispose_at', '') }}"
                            class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Remark</label>
                        <input type="text" name="remark" id="modal-remark" value="{{ old('remark', '') }}"
                            class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Dispose By (Operator)</label>
                        <select name="dispose_operator_id" id="modal-dispose-operator-id"
                            class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                            <option value="">— Pilih Operator —</option>
                            @foreach ($operators as $op)
                            <option value="{{ $op->id }}" {{ old('dispose_operator_id') == $op->id ? 'selected' : '' }}>
...
function openEditModal(btn) {
    ...
    document.getElementById('modal-dispose-at').value = d.disposeAt || '';
    document.getElementById('modal-remark').value = d.remark || '';
    document.getElementById('modal-dispose-operator-id').value = d.disposeOperatorId || '';
