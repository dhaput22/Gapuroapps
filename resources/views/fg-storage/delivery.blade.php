...
<div class="space-y-4 text-[13px] text-gray-700">
    ...
    @endphp
    <form method="GET" action="{{ route('fg.storage') }}" class="rounded-lg border border-gray-200 bg-white px-4 py-3 shadow-sm">
        ...
        <div class="mb-2 flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('fg.storage') }}" class="h-9 rounded border border-gray-300 bg-white px-3 py-2 text-xs text-gray-600 hover:bg-gray-50">
                Reset
            </a>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3 border-t border-gray-100 pt-2">
            ...
            <div class="text-xs text-gray-500">
                Total Qty Delivery : <strong class="text-gray-700">{{ number_format($summary['total_qty'] ?? 0) }}</strong>
            </div>
        </div>
    </form>
    ...
    <div class="flex flex-wrap gap-2">
        <a href="{{ route('fg.storage.delivery.scan') }}" class="inline-flex items-center gap-1.5 rounded border border-yellow-500 bg-yellow-300 px-2 py-1 text-sm font-medium text-gray-800 hover:bg-yellow-400">
                    ...
                    <td class="border border-yellow-200 px-2 py-1 text-center">
                        <div class="flex items-center justify-center gap-1">
                            <button type="button" title="Edit"
                                data-update-url="{{ route('fg.storage.delivery.update', $scan) }}"
                                data-part-code="{{ $scan->part_code }}"
                                data-part-name="{{ $scan->part_name ?? '' }}"
                                data-lot-no="{{ $scan->lot_no }}"
                                data-qty-box="{{ $scan->qty_box }}"
                                data-delivery-at="{{ optional($scan->delivery_at)->format('Y-m-d') }}"
                                data-delivery-operator-id="{{ $scan->delivery_operator_id ?? '' }}"
                                data-transfer-card-no="{{ $scan->transfer_card_no ?? '' }}"
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="w-full max-w-lg rounded border border-gray-300 bg-white shadow-xl">
            <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3">
                <h3 class="font-semibold text-gray-700">Edit FG Delivery</h3>
            ...
            <form id="editModal-form" method="POST"
                ...
                <div class="grid grid-cols-1 gap-3 px-4 py-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Delivery Date</label>
                        <input type="date" name="delivery_at" id="modal-delivery-at" value="{{ old('delivery_at', '') }}"
                            class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Transfer Card No</label>
                        <input type="text" name="transfer_card_no" id="modal-transfer-card-no" value="{{ old('transfer_card_no', '') }}"
                            class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Delivery By (Operator)</label>
                        <select name="delivery_operator_id" id="modal-delivery-operator-id"
                            class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                            <option value="">— Pilih Operator —</option>
                            @foreach ($operators as $op)
                            <option value="{{ $op->id }}" {{ old('delivery_operator_id') == $op->id ? 'selected' : '' }}>
...
function openEditModal(btn) {
    ...
    document.getElementById('modal-delivery-at').value = d.deliveryAt || '';
    document.getElementById('modal-transfer-card-no').value = d.transferCardNo || '';
    document.getElementById('modal-delivery-operator-id').value = d.deliveryOperatorId || '';
