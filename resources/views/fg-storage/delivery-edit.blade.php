...
@section('title', 'Edit FG Delivery')
...
<div class="space-y-4 text-[13px] text-gray-700">
    ...
    <h1 class="text-center text-2xl font-semibold text-gray-700">Edit FG Delivery</h1>
    ...
    <form method="POST" action="{{ route('fg.storage.delivery.update', $scan) }}" class="rounded border border-gray-200 bg-gray-50 px-6 py-5 max-w-xl">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-3">
            ...
            <div>
                ...
                <input type="date" name="delivery_at" value="{{ old('delivery_at', optional($scan->delivery_at)->format('Y-m-d')) }}"
                    class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                @error('delivery_at')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Delivery By (Operator)</label>
                <select name="delivery_operator_id" class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                    <option value="">— Pilih Operator —</option>
                    @foreach ($operators as $operator)
                        <option value="{{ $operator->id }}" {{ old('delivery_operator_id', $scan->delivery_operator_id) == $operator->id ? 'selected' : '' }}>
                            {{ $operator->employee_id }} - {{ $operator->name }}
                        </option>
                    @endforeach
                </select>
                @error('delivery_operator_id')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Transfer Card No</label>
                <input type="text" name="transfer_card_no" value="{{ old('transfer_card_no', $scan->transfer_card_no) }}"
                    class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                @error('transfer_card_no')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>
        </div>
        ...
        <div class="mt-5 flex gap-2">
            ...
            <a href="{{ route('fg.storage') }}" class="rounded border border-gray-300 bg-white px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                Batal
            </a>
        </div>
    </form>
