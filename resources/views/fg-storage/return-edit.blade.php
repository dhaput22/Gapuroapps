...
<div class="space-y-4 text-[13px] text-gray-700">
    ...
    <h1 class="text-center text-2xl font-semibold text-gray-700">Edit FG Return</h1>
    ...
    <form method="POST" action="{{ route('fg.storage.return.update', $scan) }}" class="rounded border border-gray-200 bg-gray-50 px-6 py-5 max-w-xl">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-3">
            ...
            <div>
                <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Return Date</label>
                <input type="date" name="return_at" value="{{ old('return_at', optional($scan->return_at)->format('Y-m-d')) }}"
                    class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                @error('return_at')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="mb-1 block text-xs font-semibold uppercase text-gray-500">Return By (Operator)</label>
                <select name="return_operator_id" class="h-9 w-full rounded border border-gray-300 bg-white px-2 text-sm focus:border-gray-400 focus:outline-none">
                    <option value="">— Pilih Operator —</option>
                    @foreach ($operators as $operator)
                        <option value="{{ $operator->id }}" {{ old('return_operator_id', $scan->return_operator_id) == $operator->id ? 'selected' : '' }}>
                            {{ $operator->employee_id }} - {{ $operator->name }}
                        </option>
                    @endforeach
                </select>
                @error('return_operator_id')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
            </div>
        ...
        <div class="mt-5 flex gap-2">
            ...
            <a href="{{ route('fg.storage.return.index') }}" class="rounded border border-gray-300 bg-white px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                Batal
            </a>
        </div>
    </form>
