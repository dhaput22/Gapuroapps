...
<div class="space-y-4 text-[13px] text-gray-700">
    ...
    <div class="text-center">
        ...
        <h2 class="mt-1 text-[32px] font-bold leading-tight text-gray-800">FG Dispose</h2>
    </div>
    ...
    $sortUrl = static function (string $column) use (
        ...
        $query = [
            'dispose_date' => $filters['dispose_date'] ?? null,
        ...
        return route('fg.storage.dispose.create', $query);
    };
    @endphp

    <form id="dispose-form" method="POST" action="{{ route('fg.storage.dispose.store') }}"
        class="grid gap-x-12 gap-y-3 rounded border border-gray-200 bg-gray-50 px-3 py-4 lg:grid-cols-2 lg:px-10">
        @csrf

        <div class="space-y-3">
            <div class="flex items-center">
                <label class="w-40 font-medium">Dispose Date</label>
                <span class="mr-3">:</span>
                <input type="date" name="dispose_date" value="{{ old('dispose_date', $filters['dispose_date']) }}"
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center">
                <label class="w-40 font-medium">Dispose By</label>
                <span class="mr-3">:</span>
        ...
        <table class="min-w-full border-collapse text-xs">
            <thead class="bg-yellow-200 text-gray-700">
                <tr>
                    ...
                    <th class="whitespace-nowrap border border-yellow-300 px-2 py-2 text-left">Dispose By</th>
...
<script>
    (function() {
        ...
        const partPreviewUrl = "{{ route('fg.storage.dispose.preview-part') }}";
        const scanPreviewUrl = "{{ route('fg.storage.dispose.preview') }}";
        ...
        const previewPartCode = async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    hint.textContent = payload.message || 'Part Code tidak ditemukan di FG Receiving.';
                    hint.className = 'text-xs text-red-600';
                    partNamePreview.value = '';
                    qtyPreview.value = '';
                    return false;
                ...
                hint.textContent = `Part ditemukan di FG Receiving. Part Name: ${payload.part_name}.`;
        ...
        const previewScan = async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    hint.textContent = payload.message || 'Data tidak ditemukan di FG Receiving.';
                    hint.className = 'text-xs text-red-600';
                    return false;
                }
                ...
                partNamePreview.value = payload.part_name || '';
                qtyPreview.value = payload.qty_box ?? '';
                hint.textContent = `Lot ditemukan di FG Receiving. Part: ${payload.part_name}, Qty: ${payload.qty_box}. Siap di-dispose.`;
                hint.className = 'text-xs text-green-700';
                return true;
            } catch {
                hint.textContent = 'Gagal membaca data. Coba lagi.';
